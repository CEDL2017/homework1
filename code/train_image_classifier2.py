...
# you may not  this file except in compliance  the License.
...
tf.app.flags.DEFINE_integer(
    'que_batch', 10, 'The min number of batches in each queue.')
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      provider = slim.dataset_data_provider.DatasetDataProvider(
          ...
          common_queue_capacity=2*FLAGS.que_batch * FLAGS.batch_size,
          common_queue_min=FLAGS.que_batch * FLAGS.batch_size)
      [image, label, label1, label2] = provider.get(['image', 'label', 'label1', 'label2'])
      label -= FLAGS.labels_offset
      label1 -= FLAGS.labels_offset
      label2 -= FLAGS.labels_offset
      ...
      labels = slim.one_hot_encoding(
          labels, dataset.num_classes[0] - FLAGS.labels_offset)
      labels1 = slim.one_hot_encoding(
          labels1, dataset.num_classes[1] - FLAGS.labels_offset)
      labels2 = slim.one_hot_encoding(
          labels2, dataset.num_classes[2] - FLAGS.labels_offset)
      batch_queue = slim.prefetch_queue.prefetch_queue(
          [images, labels, labels1, labels2], capacity=2 * deploy_config.num_clones)
    ...
    def clone_fn(batch_queue):
      ...
      images, labels, labels1, labels2 = batch_queue.dequeue()
      logits, logits1, logits2, end_points = network_fn(images)
    ...
    #     aggregation_method=tf.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N)  # EXPERIMENTAL_TREE
