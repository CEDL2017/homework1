...
class ScoreNet(object):
    ...
    def __init__(self, save_path='47.h5', use_VGG=False):
        ...
        if not use_VGG:
            # 1st Conv
            self.model.add(Conv2D(16, (3, 3), padding='same', name="conv1_1", activation="relu",input_shape=(150, 150, 3)))
            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(16, (3, 3), padding='same', name="conv1_2", activation="relu"))
            self.model.add(MaxPooling2D((2,2), strides=(2,2)))

            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(32, (3, 3), padding='same',name="conv2_1", activation="relu"))
            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(32, (3, 3), padding='same', name="conv2_2", activation="relu"))
            self.model.add(MaxPooling2D((2,2), strides=(2,2)))

            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(64, (3, 3), padding='same', name="conv3_1", activation="relu"))
            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(64, (3, 3), padding='same', name="conv3_2", activation="relu"))
            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(64, (3, 3), padding='same', name="conv3_3", activation="relu"))
            self.model.add(Conv2D(64, (3, 3), padding='same', name="conv3_4", activation="relu"))
            self.model.add(MaxPooling2D((2,2), strides=(2,2)))

            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(128, (3, 3), padding='same', name="conv4_1", activation="relu"))
            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(128, (3, 3), padding='same', name="conv4_2", activation="relu"))
            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(128, (3, 3), padding='same', name="conv4_3", activation="relu"))
            self.model.add(Conv2D(128, (3, 3), padding='same', name="conv4_4", activation="relu"))
            self.model.add(MaxPooling2D((2,2), strides=(2,2)))

            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(128, (3, 3), padding='same', name="conv5_1", activation="relu"))
            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(128, (3, 3), padding='same', name="conv5_2", activation="relu"))
            # self.model.add(ZeroPadding2D((1,1)))
            self.model.add(Conv2D(128, (3, 3), padding='same', name="conv5_3", activation="relu"))
            self.model.add(Conv2D(128, (3, 3), padding='same', name="conv5_4", activation="relu"))
            self.model.add(MaxPooling2D((2,2), strides=(2,2)))

            self.model.add(Flatten(name="flatten"))
            self.model.add(Dense(1024, activation='relu', name='dense_1'))
            self.model.add(Dropout(0.5))
            self.model.add(Dense(1024, activation='relu', name='dense_2'))
            self.model.add(Dropout(0.5))
            self.model.add(Dense(scorenet_fc_num, name='dense_3'))
            self.model.add(Activation('softmax',name="final_layer"))
