...
tl_model_file = os.path.join(base_folder, "Output", "TransferLearning_ResNet18_Dule.model")
...
_base_model_file = os.path.join(base_folder, "ResNet50_ImageNet_CNTK.model")
...
def create_mb_source(map_file_head, map_file_hand, image_width, image_height, num_channels, num_classes, randomize=True):
    transforms = [xforms.scale(width=image_width, height=image_height, channels=num_channels, interpolations='linear')] 
    
    image_source_head = ImageDeserializer(map_file_head, StreamDefs(
            features_head =StreamDef(field='image', transforms=transforms),
            labels_head   =StreamDef(field='label', shape=num_classes)))

    image_source_hand = ImageDeserializer(map_file_hand, StreamDefs(
            features_hand =StreamDef(field='image', transforms=transforms),
            labels   =StreamDef(field='label', shape=num_classes)))

    return MinibatchSource([image_source_head, image_source_hand],
            randomize=randomize)
...
def create_model(base_model_file, feature_node_name, last_hidden_node_name, num_classes, input_head, input_hand, freeze=False):
    ...
    z_head          = Dense(2048, activation=None, name='Head-­stream') (cloned_out_head)

    feat_norm  = input_hand - Constant(114)
    cloned_out_hand = cloned_layers_hand(feat_norm)
    z_hand          = Dense(2048, activation=None, name='Hand­‐stream') (cloned_out_hand)
...
def train_model(base_model_file, feature_node_name, last_hidden_node_name,
                image_width, image_height, num_channels, num_classes, 
                train_map_file_head, train_map_file_hand,
                num_epochs, max_images=-1, freeze=False):
    epoch_size = sum(1 for line in open(train_map_file_hand))
    if max_images > 0:
        epoch_size = min(epoch_size, max_images)

    # Create the minibatch source and input variables
    minibatch_source = create_mb_source(train_map_file_head, train_map_file_hand, image_width, image_height, num_channels, num_classes)
    image_input_head = C.input_variable((num_channels, image_height, image_width))
    image_input_hand = C.input_variable((num_channels, image_height, image_width))
    label_input = C.input_variable(num_classes)

    # Define mapping from reader streams to network inputs
    input_map = {
        image_input_head: minibatch_source['features_head'],
        image_input_hand: minibatch_source['features_hand'],
        label_input: minibatch_source[label_stream_name]
    }

    # Instantiate the transfer learning model and loss function
    tl_model = create_model(base_model_file, feature_node_name, last_hidden_node_name, num_classes, image_input_head, image_input_hand, freeze)
...
def eval_single_image(loaded_model, image_path_head, image_path_hand, image_width, image_height):
    # load and format image (resize, RGB -> BGR, CHW -> HWC)
    img = Image.open(image_path_head).convert("RGBA")
    if image_path_head.endswith("png"):
        temp = Image.new("RGBA", img.size, (255, 255, 255))
        temp.paste(img, img)
        img = temp
    resized = img.resize((image_width, image_height), Image.ANTIALIAS)
    bgr_image = np.asarray(resized, dtype=np.float32)[..., [2, 1, 0]]
    hwc_format_head = np.ascontiguousarray(np.rollaxis(bgr_image, 2))

    img = Image.open(image_path_hand).convert("RGBA")
    if image_path_hand.endswith("png"):
        temp = Image.new("RGBA", img.size, (255, 255, 255))
        temp.paste(img, img)
        img = temp
    resized = img.resize((image_width, image_height), Image.ANTIALIAS)
    bgr_image = np.asarray(resized, dtype=np.float32)[..., [2, 1, 0]]
    hwc_format_hand = np.ascontiguousarray(np.rollaxis(bgr_image, 2))
    ...
    arguments = {loaded_model.arguments[0]: [hwc_format_head], loaded_model.arguments[1]: [hwc_format_hand]}
